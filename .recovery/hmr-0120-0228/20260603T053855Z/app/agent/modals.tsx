...
export function BudgetModal(props: {
  ...
  const [budgetViewMode, setBudgetViewMode] = useState<1 | 2>(1);
  ...
  return (
    <div className="agent-modal-overlay budget-modal-overlay" onClick={props.onClose}>
      ...
      >
        {/* ── HEADER ─────────────────────────────────── */}
        <div className="bcc-modal-header bm-header">
          ...
          <div className="bm-header-kpis">
            <span className={`bm-kpi-pill ${heroToneClass}`}>
              {props.budgetSignals.balanceLabel}
            </span>
            <span className="bm-kpi-pill bm-kpi-neutral">
              {props.budgetCompletion.fillRate}%
            </span>
          </div>
          <button
            type="button"
        ...
        {flyingDots.map((dot) => (
          <div key={dot.id} className={`budget-dot-fly budget-dot-fly-${dot.type}`} />
        ))}

        {/* ── TABS (mobile only) ───────────────────── */}
        <div className="bm-tab-nav" role="tablist" aria-label="Sección de presupuesto">
          <button
            type="button"
            role="tab"
            aria-selected={budgetViewMode === 1}
            className={`bm-tab${budgetViewMode === 1 ? ' is-active' : ''}`}
          ...
          <button
            ...
            className={`bm-tab${budgetViewMode === 2 ? ' is-active' : ''}`}
        ...
        {/* ── BODY: two panels ────────────────────── */}
        <div className={`bm-body${isDesktopLayout ? ' is-desktop' : ''}`}>

          {/* Panel 1 — Agent chat */}
          <section
            className="bm-panel bm-panel--agent"
            aria-label="Asistente de presupuesto"
            hidden={!isDesktopLayout && budgetViewMode !== 1}
          >
            <div className="bm-agent-inner">
              <div className="bm-agent-question-wrap">
                {props.sessionInfo?.injectedIntake && (
                  <span className="bm-profile-pill">Perfil activo</span>
                )}
                <p className="bm-question">
                  {isInitializing
                    ? 'Preparando conversación con contexto de tabla…'
                    : isAskingAI
                      ? 'Analizando tabla y mensaje…'
                      : activeQuestion}
                </p>
                {isAskingAI && (
                  <span className="bm-thinking" aria-live="polite" aria-label="Pensando">
                ...
                {conversationDone && (
                  <p className="bm-done">✓ Presupuesto completo — ajusta la tabla cuando quieras.</p>
                )}
                {assistantCoachMessage && (
                  <p className="bm-coach">{assistantCoachMessage}</p>
                )}
                {props.coachHint && !assistantCoachMessage && !conversationDone && (
                  <p className="bm-tip">{props.coachHint}</p>
                )}
              </div>
              ...
              <div className="bm-input-row">
                <input
                  ...
                  placeholder="Responde o pide cambios a la tabla"
                  autoFocus
                ...
                >
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                    <path d="M2 8h12M8 3l6 5-6 5" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </button>
              </div>

              {aiError && <p className="bm-error" role="alert">{aiError}</p>}

              <button
                type="button"
                className="bm-inject-btn"
                onClick={props.sendBudgetToAgent}
              >
                Inyectar a Financieramente
              </button>
            </div>
          </section>
          ...
          {/* Panel 2 — Table */}
          <section
            className="bm-panel bm-panel--table"
            aria-label="Tabla de presupuesto"
            hidden={!isDesktopLayout && budgetViewMode !== 2}
          >
            <div className="bm-table-topbar">
              <div className="bm-table-add-btns">
                <button
                  type="button"
                  className="continue-ghost bm-add-income"
                  onClick={() => props.addBudgetRow('income')}
                >
                  + Ingreso
                </button>
                <button
                  type="button"
                  className="continue-ghost bm-add-expense"
                  onClick={() => props.addBudgetRow('expense')}
                >
                  + Gasto
                </button>
              </div>
              <div className="bm-table-util-btns">
                <button
                  type="button"
                  className="continue-ghost bm-style-btn"
                  onClick={cycleBudgetTableStyle}
                >
                  {activeStyleLabel}
                </button>
                <button
                  type="button"
                  className="continue-ghost bm-pdf-btn"
                  onClick={() => void downloadBudgetPdf()}
                  disabled={isGeneratingBudgetPdf || props.budgetRows.length === 0}
                >
                  {isGeneratingBudgetPdf ? 'PDF…' : 'PDF'}
                </button>
              </div>
            </div>
                    ...
                    <div className="pt-stack-carousel tx-library-is-saved">
                      ...
                      return (
                        ...
                        >
                          ...
                          {isTop && libraryProductCards.length > 1 ? (
                            <div className="tx-lib-card-nav">
                              <button
                                type="button"
                                className="continue-ghost"
