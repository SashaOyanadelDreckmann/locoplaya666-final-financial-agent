...
import { isTabletPortraitBrowser } from '@/lib/interfaz/viewport-mode';
import {
...
export default function BrowserChromeVignetteSync() {
  useEffect(() => {
    ...
    const syncOnScroll = () => {
      // Home scrolls the document for Framer useScroll — skip per-frame token churn.
      if (document.documentElement.classList.contains('home-route-active')) return;
      sync();
    };
    ...
    vv?.addEventListener('scroll', syncOnScroll);
    ...
    return () => {
      ...
      vv?.removeEventListener('scroll', syncOnScroll);
