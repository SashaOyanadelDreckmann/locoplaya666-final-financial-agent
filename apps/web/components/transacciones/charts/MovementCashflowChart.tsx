...
function MovementRow(props: { movement: TransactionCashflowMovement; formatCurrency: (value: number) => string }) {
...
export function MovementCashflowChart(props: {
  ...
  return (
    <section
      className={`tx-minimal-cashflow-card${props.className ? ` ${props.className}` : ''}`}
      aria-label={`Evolución acumulada de ingresos y egresos en ${series.monthLabel}`}
    >
      <div className="tx-minimal-cashflow-head">
        <div>
          <span className="tx-minimal-cashflow-kicker">{props.kicker ?? 'Evolución acumulada'}</span>
          <h3 className="tx-minimal-cashflow-title">{props.title ?? series.monthLabel}</h3>
      ...
      <div className="tx-minimal-cashflow-chart">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={series.points} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}>
            ...
            />
            <Area type="monotone" dataKey="incomeArea" stroke="transparent" fill={`url(#${gradientId})`} dot={false} isAnimationActive={false} />
            <Line
              type="monotone"
