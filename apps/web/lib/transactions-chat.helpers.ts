...
function movementKey(row: MovementRow): string {
  return [
    String(row.date ?? ''),
    String(row.merchant ?? row.description ?? row.label ?? ''),
    String(row.amount ?? ''),
  ].join('|');
}

/** Prioriza montos altos, ingresos, recientes y variedad — no solo los primeros N cronológicos. */
export function selectMovementsForPrompt(movements: MovementRow[], maxMovements: number): MovementRow[] {
  ...
  take(
    movements
      .filter((row) => !picked.has(movementKey(row)))
      .sort((a, b) => movementAmount(b) - movementAmount(a)),
  );
...
export function extractQuestionSignals(question: string): QuestionSignals {
  ...
  const amounts = Array.from(
    new Set(
      [...question.matchAll(/(?:\$|\bclp\b\s*)?(\d{1,3}(?:[.\s]\d{3})+|\d{4,8})(?:\s*clp)?/gi)]
  ...
  for (const match of question.matchAll(
    ...
    dates.push({
      day: Number(match[1]),
      month: SPANISH_MONTHS[normalizeSearchText(match[2])],
...
function movementSearchBlob(row: MovementRow): string {
  return normalizeSearchText(
    [row.date, row.merchant, row.description, row.label, row.category].filter(Boolean).join(' '),
  );
}
...
function scoreMovementForQuestion(row: MovementRow, signals: QuestionSignals): number {
  ...
  for (const dateSignal of signals.dates) {
    if (movementMatchesDate(row, dateSignal)) score += 8;
  }
...
function isOverviewQuestion(signals: QuestionSignals): boolean {
  const hasSpecificSignal =
    signals.keywords.length > 0 || signals.amounts.length > 0 || signals.dates.length > 0;
...
export function retrieveMovementsForQuestion(
  ...
  if (movements.length === 0) {
    return { movements: [], mode: 'overview', matchedCount: 0, signals };
  }

  if (isOverviewQuestion(signals)) {
    return {
      movements: selectMovementsForPrompt(movements, overviewSample),
      mode: 'overview',
      matchedCount: 0,
      signals,
    };
  ...
  const ranked = movements
    .map((row) => ({ row, score: scoreMovementForQuestion(row, signals) }))
    .filter((entry) => entry.score >= minScore)
    .sort((a, b) => b.score - a.score || movementAmount(b.row) - movementAmount(a.row));
  ...
  if (targeted.length >= maxRetrieved) {
    return {
      movements: targeted,
      mode: 'targeted',
  ...
  const supplement = selectMovementsForPrompt(
    movements.filter((row) => !targetedKeys.has(movementKey(row))),
    Math.max(0, overviewSample - targeted.length),
  );

  return {
    movements: [...targeted, ...supplement],
    mode: 'targeted',
    matchedCount: ranked.length,
    signals,
  };
...
function compactMovementRow(row: MovementRow) {
  return {
    date: row.date ?? '',
    description: compactTxText(row.description ?? row.label ?? '', 96),
    amount: row.amount ?? 0,
    direction: row.direction ?? 'expense',
    category: compactTxText(row.category ?? '', 48),
    merchant: compactTxText(row.merchant ?? '', 48),
  };
}

export function buildChatDashboardForQuestion(
  dashboard: unknown,
  question: string,
  options?: { maxRetrieved?: number; overviewSample?: number },
): LooseRecord | null {
  ...
  return {
    ...
    topCategories: Array.isArray(d.topCategories) ? d.topCategories.slice(0, 8) : [],
    topMerchants: Array.isArray(d.topMerchants) ? d.topMerchants.slice(0, 10) : [],
    spendClusters: Array.isArray(d.spendClusters) ? d.spendClusters.slice(0, 4) : [],
    alerts: Array.isArray(d.alerts) ? d.alerts.slice(0, 3) : [],
    retrieval: {
      mode: retrieval.mode,
...
export function compactDashboardForPrompt(
  dashboard: unknown,
  options?: { maxMovements?: number; maxMerchants?: number },
): LooseRecord | null {
  if (!dashboard || typeof dashboard !== 'object') return null;
  const d = dashboard as LooseRecord;
  ...
  return {
    ...
    keyMetrics: d.keyMetrics ?? null,
    topCategories: Array.isArray(d.topCategories) ? d.topCategories.slice(0, 8) : [],
    topMerchants: Array.isArray(d.topMerchants) ? d.topMerchants.slice(0, maxMerchants) : [],
    spendClusters: Array.isArray(d.spendClusters) ? d.spendClusters.slice(0, 6) : [],
    alerts: Array.isArray(d.alerts) ? d.alerts.slice(0, 4) : [],
    alertDetails: Array.isArray(d.alertDetails) ? d.alertDetails.slice(0, 4) : [],
    movements: selectedMovements.map((row) => compactMovementRow(row)),
  };
}

export function compactDocumentsForPrompt(
  docs: Array<{
    documentId?: string;
    name?: string;
    text?: string;
  ...
  options?: { maxDocs?: number; maxText?: number },
) {
...
export function compactChatHistory(
  ...
  return messages.slice(-maxMessages).map((message) => ({
    ...
    text: compactTxText(message.text ?? '', maxText),
  }));
}
