...
/** Safe arithmetic evaluator — no variables, no eval(). */
...
/** Substitute {step_id} placeholders with numeric results from prior steps. */
export function resolveExpressionRefs(
  expression: string,
  refs: Record<string, number>,
): string {
  return expression.replace(/\{([a-zA-Z][a-zA-Z0-9_]*)\}/g, (_match, key: string) => {
