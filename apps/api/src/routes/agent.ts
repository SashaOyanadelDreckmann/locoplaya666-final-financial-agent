...
function buildContextualSuggestedReplies(params: {
  ...
  if (activeChatId === 'chat-2') {
    return ['Volver al plan', 'Priorizar deudas', 'Simular ahorro'];
  }
  if (activeChatId === 'chat-3') {
    return ['Resumir diagnóstico', 'Revisar riesgo', 'Cerrar plan'];
  }
  if (params.hasBudget && params.hasTransactions) {
    return ['Abrir entrevista breve', 'Revisar presupuesto', 'Ver cartola'];
  }
  if (phase === 'transactions_needed') {
    return ['Subir cartola', 'Explorar deuda', 'Simular ahorro'];
  }
  if (phase === 'budget_needed') {
    return ['Completar presupuesto', 'Ver balance', 'Probar escenario'];
  }
